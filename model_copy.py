...
class Generator(nn.Module):
    def __init__(self, vocab_size, nz=100, ngf=64, v_feat_size=512, hidden_size=2*512, word_emb_size=256, max_sen=10, max_word=20):
        super(Generator, self).__init__()
...
class Discriminator(nn.Module):
    def __init__(self, vocab_size, ndf=64, word_emb_size=512, hidden_size=2*512, v_feat_size=512):
        super(Discriminator, self).__init__()
        # # D_rpt
        # self.embed = nn.Embedding(vocab_size, word_emb_size) # ? padding_value = 1
        ...
        self.D_img = D_img_Net(ResBlockDown, [1, 1, 1, 1, 1], 1)
        # # D_joint
